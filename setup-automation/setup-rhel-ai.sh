...
until ssh satellite.lab 'echo ok' &>/dev/null; do
  if (( SECONDS >= 30 )); then
    echo "ERROR: satellite.lab not reachable after 30 seconds" >> /tmp/setup-scripts/setup-rhel-ai.log
    break
  fi
  echo "Waiting for satellite.lab to come up..." >> /tmp/setup-scripts/setup-rhel-ai.log
  sleep 5
done && SATELLITE_UP=true

if $SATELLITE_UP; then
  # SSH to Satellite, create admin Hammer access token for MCP.
  FOREMAN_TOKEN=$(ssh satellite.lab 'hammer user access-token create --user=admin --name="mcp server"')
